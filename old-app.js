...
function PizzaShop (locationName, storeData) {
  ...
  this.createTable = function(inputHeaderRowArray, inputRowArray) {
    var headerRow = document.createElement('tr');
    for(var i = 0; i < this.inputHeaderRowArray.length; i++) {
      var column = document.createElement('td');
      column.textContent = inputHeaderRowArray[i];
      this.headerRow.appendChild(column);
    }
    console.this.table(inputHeaderRowArray);
    return headerRow;

    for(var i = 0; i < this.inputRowArray.length; i++) {
      var column = document.createElement('td');
      column.textContent = inputRowArray[i];
      this.row.appendChild(column);
    }
    console.this.table(inputRowArray);
    return row;
  };
...
var headerRowOne = ['Time', 'Min Pizza/hr', 'Max Pizza/hr', 'Min Delivery/hr', 'Max Delivery/hr'];
